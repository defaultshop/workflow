...
export default {
  ...
  methods: {
    ...
    continubpmn(data){
      this.$nextTick(()=>{
        ...
        // console.log(Base64.decode(workflowinfo.definitions[0].bpmnText),'bptext');
        const bpmnText = Base64.decode(data);
        console.log(bpmnText,2);
        this.saveloadXML(bpmnText);
        this.$forceUpdate();
        // this.loadXML(bpmnText);
        // undoRedoStore.dispatch('pushState', bpmnText);
      });
    },
    async saveBpmn() {
    ...
    async saveloadXML(xml = this.currentXML) {
  ...
  created() {
    ...
    console.log(this.xmlManager,'xmlManager');
