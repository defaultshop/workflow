...
import expressionAccordionConfig from '@/components/inspectors/expressionAccordionConfig';
...
export default {
  ...
  computed: {
    ...
    config() {
      ...
      const sequenceFlowConfigurationFormElements = get(inspectorConfig, '[0].items');
      ...
      if (this.isSequenceFlow(type) && this.isConnectedToGateway(definition)) {
        ...
        //   },
        // };
        // const nameConfigSettings =  get(inspectorConfig,'[0].items[0].items[0]');
        // nameConfigSettings.name = '分支';
        // console.log(nameConfigSettings, 'configSettings');
        
        sequenceFlowConfigurationFormElements.splice(1,0,expressionAccordionConfig);

        // var array = [];
        // array.push(expressionAccordionConfig);
        // array.push(sequenceFlowConfigurationFormElements);

        /* 破坏性变更，这里应该加入自定义的控件来支持表达式自定义 */
      }
      else if (this.isSequenceFlow(type))
      {
        var nameConfig = sequenceFlowConfigurationFormElements[0].config;
        nameConfig.initiallyOpen = true;
      }
