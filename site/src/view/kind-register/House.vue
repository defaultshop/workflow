        ...
        <Form label-position="top">
          ...
          <Row>
            ...
            <Col span="11" offset="2" v-show="form.houseStatus === 1">
              <FormItem label="不动产权号">
                <Input v-model.trim="form.houseRealEstateCertificateNo" placeholder="请输入" :maxlength="50"></Input>
              </FormItem>
            </Col>
          </Row>
          <Row>
          ...
          <Row>
            ...
            <Col span="11" v-show="form.houseStatus === 1">
              <FormItem label="请上传产权证明（所提供房产须与所报志愿园区相符）">
...
export default {
  ...
  methods: {
    getEditData() {
      ...
      UnSubmittedSchoolRegisterInfo(params).then(res => {
        if (res.success) {
          ...
          this.form.houseRegisterAddress = res.result.houseRegisterAddress;
    ...
    async submitInfo() {
      ...
      if (!/(^\d{17}[\dXx]$)/.test(this.form.houseCardId)) return this.$Message.error('产权人身份证号格式不正确!');
