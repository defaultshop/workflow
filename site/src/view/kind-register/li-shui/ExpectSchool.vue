    ...
    <div class="content-box">
      <div class="form-list">
        ...
        <Form label-position="top">
          ...
          <Row>
            <Col v-if="showOtherSchool && businessType === 201">
          ...
          <Row>
            <Col v-if="showOtherSchool && businessType === 201">
              <FormItem label="调配志愿三">
                <div class="select-school">
                  <RadioGroup v-model="form.schoolInfoThreeId">
                    <Radio :label="item.id" v-for="item in schoolList" :key="item.id">{{ item.name }}</Radio>
                  </RadioGroup>
                </div>
              </FormItem>
          ...
          <Row>
            <Col v-if="showOtherSchool && businessType === 201">
              <FormItem label="调配志愿四">
                <div class="select-school">
                  <RadioGroup v-model="form.schoolInfoFourId">
                    <Radio :label="item.id" v-for="item in schoolList" :key="item.id">{{ item.name }}</Radio>
                  </RadioGroup>
                </div>
              </FormItem>
          ...
          <Row>
            <Col v-if="showOtherSchool && businessType === 201">
              <FormItem label="调配志愿五">
                <div class="select-school">
                  <RadioGroup v-model="form.schoolInfoFiveId">
                    <Radio :label="item.id" v-for="item in schoolList" :key="item.id">{{ item.name }}</Radio>
                  </RadioGroup>
                </div>
              </FormItem>
          ...
          <Row>
            <Col v-if="showOtherSchool && businessType === 201">
              <FormItem label="调配志愿六">
                <div class="select-school">
                  <RadioGroup v-model="form.schoolInfoSixId">
                    <Radio :label="item.id" v-for="item in schoolList" :key="item.id">{{ item.name }}</Radio>
                  </RadioGroup>
                </div>
              </FormItem>
  ...
  methods: {
    getEditData() {
      ...
      UnSubmittedSchoolRegisterInfo(params).then(res => {
        if (res.success) {
          ...
          this.form.schoolInfoThreeId = res.result.schoolInfoThreeId;
          this.form.schoolInfoFourId = res.result.schoolInfoFourId;
          this.form.schoolInfoFiveId = res.result.schoolInfoFiveId;
          this.form.schoolInfoSixId = res.result.schoolInfoSixId;
        }
      });
    },
    async getExpectSchool() {
      let res = await referens.get_GetEnableList({
        schoolType: 1,
        schoolRegisterType: this.schoolRegisterType,
        isSubset: this.businessType === 202 ? true : false
      });
      if (res.success) {
    ...
    expectScoolChange(val) {
      ...
      this.form.schoolInfoThreeId = 0;
      this.form.schoolInfoFourId = 0;
      this.form.schoolInfoFiveId = 0;
      this.form.schoolInfoSixId = 0;
